...
export function VideoText({
  ...
  useEffect(() => {
    const updateSvgMask = () => {
      // Responsive font sizing - make text bigger on mobile
      let responsiveFontSize;
      if (typeof fontSize === "number") {
        // On mobile (smaller screens), increase font size
        if (window.innerWidth < 768) {
          // Mobile: increase font size by 2x
          responsiveFontSize = `${fontSize * 2}vw`;
        } else if (window.innerWidth < 1024) {
          // Tablet: increase font size by 1.5x
          responsiveFontSize = `${fontSize * 1.5}vw`;
        } else {
